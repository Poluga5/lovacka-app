...
const NAV = [
  { href: '/dashboard',             label: 'Naslovna',    icon: 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6' },
  ...
  { href: '/dashboard/lovci',       label: 'Lovci',       icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-64 bg-forest-950 text-white flex-shrink-0">
        <div className="p-5 border-b border-forest-800">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-semibold text-sm leading-tight truncate">{clubName}</p>
        ...
        <div className="p-4 border-t border-forest-800">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-8 h-8 bg-forest-600 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">
      ...
      {/* Mobile menu overlay */}
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-forest-950 pt-14 px-4 overflow-y-auto">
              ...
              return (
                <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-3 py-3 rounded-xl text-sm ${
                    isActive ? 'bg-forest-600 text-white font-medium' : 'text-forest-300'
                  }`}>
                  <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={item.icon} />
                  </svg>
          ...
          <div className="border-t border-forest-800 pt-4 pb-8">
            ...
            <button onClick={handleLogout}
              className="w-full text-left text-sm text-forest-400 px-3 py-2 rounded-lg hover:bg-forest-800">
              Odjava
            </button>
          </div>
        </div>
