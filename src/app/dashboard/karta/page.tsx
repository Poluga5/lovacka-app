...
export default function KartaPage() {
  ...
  const [editPOI, setEditPOI] = useState({ name: '', type: 'ceka', description: '', zaduzeni: '', zapazanja: '', zadaci: '' })
  ...
  const [activeTab, setActiveTab] = useState<'info' | 'rezervacije' | 'zadaci'>('info')
  ...
  function getMarkerColor(poiId: string, type: string, reservations: any[]): string {
    ...
    const todayRes = reservations.filter(r =>
      r.poi_id === poiId &&
      r.status === 'aktivna' &&
      isToday(parseISO(r.date_start))
    )
    const futureRes = reservations.filter(r =>
      r.poi_id === poiId &&
      r.status === 'aktivna' &&
      new Date(r.date_start) > today
    )
    if (todayRes.length > 0) return '#F97316' // narančasta = rezervirano danas
    if (futureRes.length > 0) return '#EAB308' // žuta = ima buduću rezervaciju
    return type === 'ceka' ? '#DC2626' : // crvena = slobodna čeka
           type === 'hraniliste' ? '#16A34A' :
           type === 'soliste' ? '#D97706' :
           type === 'kaljuziste' ? '#2563EB' :
           type === 'prolaz' ? '#7C3AED' :
           type === 'kamera' ? '#0891B2' : '#6B7280'
  }
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      const [poisRes, areasRes, resRes] = await Promise.all([
        ...
        supabase.from('reservations').select('*, profiles(full_name)')
  ...
  function addPOIMarker(L: any, map: any, poi: POI, reservations: any[]) {
  ...
  function updateMarkerColor(poi: POI, reservations: any[]) {
  ...
  function drawArea(L: any, map: any, area: any) {
    try {
      ...
      L.polygon(geo.coordinates[0].map((p: number[]) => [p[1], p[0]]), {
  ...
  async function selectPOI(poi: POI) {
    ...
    const { data } = await supabase
      ...
      .eq('poi_id', poi.id).eq('status', 'aktivna')
      .gte('date_end', new Date().toISOString())
      .order('date_start')
  ...
  async function savePOI() {
    ...
    if (mapRef.current) {
      const L = (await import('leaflet')).default
      addPOIMarker(L, mapRef.current, data, allReservations)
    }
  }
  ...
  async function updatePOI() {
    ...
    const { error } = await supabase.from('poi').update({
      ...
      zaduzeni: editPOI.zaduzeni.split(',').map(s => s.trim()).filter(Boolean),
      zapazanja: editPOI.zapazanja,
      zadaci: editPOI.zadaci,
      updated_at: new Date().toISOString()
    }).eq('id', selectedPOI.id)
    if (error) { toast.error('Greška'); return }
    toast.success('POI ažuriran!')
    const updated = { ...selectedPOI, ...editPOI, zaduzeni: editPOI.zaduzeni.split(',').map(s => s.trim()).filter(Boolean) }
    setPois(p => p.map(poi => poi.id === selectedPOI.id ? updated as POI : poi))
    setSelectedPOI(updated as POI)
    setMode('view')
  }
  ...
  async function createReservation() {
    ...
    if (error) {
      if (error.code === '23505') toast.error('Već rezervirano u tom terminu!')
    ...
    const { data } = await supabase.from('reservations').select('*, profiles(full_name)')
      .eq('poi_id', selectedPOI.id).eq('status', 'aktivna')
      .gte('date_end', new Date().toISOString()).order('date_start')
    const newReservations = data ?? []
    setPoiReservations(newReservations)
    const updatedAll = [...allReservations.filter(r => r.poi_id !== selectedPOI.id), ...newReservations]
  ...
  async function cancelReservation(id: string) {
    ...
    if (selectedPOI) {
      ...
      const newReservations = data ?? []
      setPoiReservations(newReservations)
      const updatedAll = [...allReservations.filter(r => r.id !== id)]
      setAllReservations(updatedAll)
      updateMarkerColor(selectedPOI, updatedAll)
    }
  }
  ...
  async function saveBoundary() {
    ...
    const { error } = await supabase.from('areas').insert({
      ...
      geojson_cache: JSON.stringify({ type: 'Polygon', coordinates: [closed] }),
      created_by: user.id,
    })
    if (error) { toast.error('Greška'); return }
  ...
  const tabStyle = (tab: string) => ({
    padding: '6px 12px', fontSize: 12, border: 'none', cursor: 'pointer', borderRadius: 6,
    background: activeTab === tab ? '#247a4b' : '#f3f4f6',
    color: activeTab === tab ? 'white' : '#374151', fontWeight: activeTab === tab ? 600 : 400
  })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 60px)' }}>
      ...
      <div style={{ position: 'relative', flex: 1 }}>
        ...
        {selectedPOI && mode === 'view' && (
          <div style={{ position: 'absolute', bottom: 16, right: 16, background: 'white', borderRadius: 16, width: 310, zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', maxHeight: '75vh', display: 'flex', flexDirection: 'column' }}>
            {/* Header */}
            <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: getMarkerColor(selectedPOI.id, selectedPOI.type, allReservations), fontWeight: 600, marginTop: 2 }}>
                      {allReservations.filter(r => r.poi_id === selectedPOI.id && isToday(parseISO(r.date_start))).length > 0
                        ? '🟠 Rezervirano danas'
                        : allReservations.filter(r => r.poi_id === selectedPOI.id).length > 0
                        ? '🟡 Ima rezervacije'
                        : '🟢 Slobodna'}
                    </div>
                  </div>
                </div>
                <button onClick={() => { setSelectedPOI(null); setPoiReservations([]) }}
              ...
              <div style={{ display: 'flex', gap: 4, marginTop: 10 }}>
                ...
                <button style={tabStyle('rezervacije')} onClick={() => setActiveTab('rezervacije')}>
                  Rezervacije {poiReservations.length > 0 && `(${poiReservations.length})`}
                </button>
                <button style={tabStyle('zadaci')} onClick={() => setActiveTab('zadaci')}>Zadaci</button>
              </div>
            </div>
            ...
            <div style={{ padding: '12px 20px', overflowY: 'auto', flex: 1 }}>
              ...
              {activeTab === 'info' && (
                <div>
                  ...
                  {(selectedPOI as any).zaduzeni?.length > 0 && (
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', marginBottom: 4 }}>ZADUŽENI</div>
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        {(selectedPOI as any).zaduzeni.map((z: string, i: number) => (
                          <span key={i} style={{ fontSize: 12, background: '#e0f2fe', color: '#0369a1', padding: '2px 8px', borderRadius: 20 }}>{z}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {(selectedPOI as any).zapazanja && (
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: '#6b7280', marginBottom: 4 }}>ZAPAŽANJA</div>
                      <p style={{ fontSize: 13, color: '#374151', margin: 0, background: '#fffbeb', padding: '8px 10px', borderRadius: 8 }}>{(selectedPOI as any).zapazanja}</p>
                    </div>
                  )}
                  {isAdmin && (
                    <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                      <button onClick={() => {
                        const p = selectedPOI as any
                        setEditPOI({ name: p.name, type: p.type, description: p.description ?? '', zaduzeni: (p.zaduzeni ?? []).join(', '), zapazanja: p.zapazanja ?? '', zadaci: p.zadaci ?? '' })
                        setMode('edit_poi')
                      }} style={{ flex: 1, padding: '7px', background: '#f3f4f6', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>✏️ Uredi</button>
                      <button onClick={() => deletePOI(selectedPOI)}
                        style={{ flex: 1, padding: '7px', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>🗑 Obriši</button>
                    </div>
                  )}
                </div>
              )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 8 }}>
                    {selectedPOI.type === 'ceka' && (
                      <button onClick={() => setShowNewRes(!showNewRes)}
                        style={{ fontSize: 12, padding: '5px 12px', background: '#247a4b', color: 'white', border: 'none', borderRadius: 6, cursor: 'pointer' }}>
                        + Nova rezervacija
                      </button>
                    )}
                  </div>
              ...
              {activeTab === 'zadaci' && (
                <div>
                  {(selectedPOI as any).zadaci ? (
                    <div style={{ background: '#fff7ed', borderRadius: 8, padding: '10px 12px', marginBottom: 12 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: '#9a3412', marginBottom: 4 }}>📋 ZADACI</div>
                      <p style={{ fontSize: 13, color: '#374151', margin: 0, whiteSpace: 'pre-wrap' }}>{(selectedPOI as any).zadaci}</p>
                    </div>
                  ) : (
                  ...
                  {isAdmin && (
                    <button onClick={() => {
                      ...
                      setMode('edit_poi')
                      setActiveTab('zadaci')
                    }} style={{ marginTop: 12, width: '100%', padding: '8px', background: '#f3f4f6', border: 'none', borderRadius: 8, fontSize: 13, cursor: 'pointer' }}>
                      ✏️ Uredi zadatke
                    </button>
                  )}
                </div>
        ...
        {selectedPOI && mode === 'edit_poi' && (
          <div style={{ position: 'absolute', bottom: 16, right: 16, background: 'white', borderRadius: 16, padding: 20, width: 310, zIndex: 1000, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', maxHeight: '75vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 3 }}>Naziv</label>
              ...
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 3 }}>Vrsta</label>
              ...
              <div>
                ...
                <textarea value={editPOI.description} onChange={e => setEditPOI(p => ({...p, description: e.target.value}))}
                  style={{ width: '100%', border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 12px', fontSize: 14, boxSizing: 'border-box', resize: 'none' }} rows={2} />
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 3 }}>Zaduženi lovci (odvojeni zarezom)</label>
                <input value={editPOI.zaduzeni} onChange={e => setEditPOI(p => ({...p, zaduzeni: e.target.value}))}
                  style={{ width: '100%', border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 12px', fontSize: 14, boxSizing: 'border-box' }}
                  placeholder="npr. Pero Perić, Ivek Ivić" />
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 3 }}>Zapažanja</label>
                <textarea value={editPOI.zapazanja} onChange={e => setEditPOI(p => ({...p, zapazanja: e.target.value}))}
                  ...
                  rows={2} placeholder="Npr. viđena srna u sumrak..." />
              </div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 600, color: '#374151', display: 'block', marginBottom: 3 }}>Zadaci / što treba napraviti</label>
                <textarea value={editPOI.zadaci} onChange={e => setEditPOI(p => ({...p, zadaci: e.target.value}))}
                  style={{ width: '100%', border: '1px solid #e5e7eb', borderRadius: 8, padding: '8px 12px', fontSize: 14, boxSizing: 'border-box', resize: 'none' }}
                  rows={3} placeholder="Npr. Popraviti pod čeke, napuniti hranilište..." />
              </div>
